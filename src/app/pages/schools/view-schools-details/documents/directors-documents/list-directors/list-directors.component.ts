...
export class ListDirectorsComponent implements OnInit {
  ...
  loadAllDocumentType() {
    ...
    this.httpService
      .postReq("portal/api/v1/documents/directors/getall", model)
  ...
  loadDocumentData(id: number): any {
    ...
    return this.httpService
      .postReq("portal/api/v1/documents/directors/getall/docs", model)
      .subscribe(
        (res: any) => {
          if (res.status === 0) {
            this.loading = false;
            this.directorDetails = res.data;
          } else {
            this.loading = false;
          }
        },
